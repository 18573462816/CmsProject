...
func mvcHandle(app *iris.Application) {
	...
	//获取商铺详细信息
	app.Get("/shopping/restaurant/{shop_id}", func(context context.Context) {
		shopId := context.Params().Get("shop_id")
		var shop model.Shop
		_, err := engine.Where(" shop_id = ? ", shopId).Get(&shop)
		if err != nil {
			context.JSON(iris.Map{
				"status":  utils.RECODE_FAIL,
				"type":    utils.RESPMSG_ERROR_USERINFO,
	...
	app.Post("/admin/update/avatar/{adminId}", func(context context.Context) {
		...
		out, err := os.OpenFile("./CmsProject/uploads/"+fname, os.O_WRONLY|os.O_CREATE, 0666)
		if err != nil {
			iris.New().Logger().Info(err.Error())
			context.JSON(iris.Map{
				"status":  utils.RECODE_FAIL,
	...
	app.Post("/v1/addimg/{model}", func(context context.Context) {
		...
		isExist, err := utils.PathExists("./CmsProject/uploads/" + model)

		if !isExist {
			err := os.Mkdir("./CmsProject/uploads/"+model, 0777)
			if err != nil {
				context.JSON(iris.Map{
					"status":  utils.RECODE_FAIL,
					"type":    utils.RESPMSG_ERROR_PICTUREADD,
		...
		out, err := os.OpenFile("./CmsProject/uploads/"+model+"/"+fname, os.O_WRONLY|os.O_CREATE, 0666)
		if err != nil {
			iris.New().Logger().Info(err.Error())
			context.JSON(iris.Map{
				"status":  utils.RECODE_FAIL,
...
func newApp() *iris.Application {
	...
	//设定应用图标
	app.Favicon("./CmsProject/static/favicons/favicon.ico")
	...
	app.HandleDir("/img","./CmsProject/uploads")
